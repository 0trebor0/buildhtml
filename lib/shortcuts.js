...
function applyShortcuts(proto, createChildKey) {
  ...
  proto.select = function (optionsList, attrs) {
    ...
    for (const opt of optionsList) {
      ...
      // A plain string or number used to reach the object reads below and emit
      // <option></option> — an option with no value and no label, which is
      // silently wrong rather than a failure. Treat it as its own value and text.
      if (typeof opt !== 'object') {
        el.child('option').attr('value', String(opt)).text(String(opt));
        continue;
      }
  ...
  proto.container = function (childFn, maxWidth) {
    const c = this[createChildKey]('div');
    if (maxWidth !== undefined) c.css({ maxWidth });
    if (typeof childFn === 'function') childFn(c);
    return c;
  };
  ...
  proto.dataTable = function (headers, rows, options) {
    ...
    if (!options || typeof options !== 'object') options = {};
    // A truthy non-array headers value used to reach the row loop intact: a
    // number or object threw "keys is not iterable", and a string silently
    // spread into one column per character. Only an array names columns.
    if (!Array.isArray(headers)) headers = null;
    const tbl = this[createChildKey]('table');
    if (options.class) tbl.addClass(options.class);
    if (!headers && options.autoHeaders && rows.length > 0 && rows[0] && typeof rows[0] === 'object') {
