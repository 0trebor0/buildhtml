...
/* ---- FNV-1a Hash ----
 *
 * Two FNV-1a lanes over the same input, differing only in their offset basis,
 * concatenated in base 36. The single 32-bit lane this replaced collided at the
 * rate the birthday bound predicts — measured at 4 collisions over 150,000
 * distinct declaration blocks and 17 over 400,000 — and a collision here is not
 * a hash-table miss that gets retried. It is two different rules sharing one
 * class name, which renders as silently wrong styling with nothing to log.
 *
 * Even a page with a few thousand rules sat around one chance in three hundred
 * of carrying one. Two lanes take that to roughly one in 10^9 at 400,000 rules,
 * for six more characters per class name — characters that repeat across every
 * element using the rule and compress accordingly.
 *
