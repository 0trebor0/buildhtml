...
/* ---- FNV-1a Hash ----
 *
 * One full FNV-1a lane in base 36, plus the low three base-36 digits of a second
 * lane that differs only in its offset basis: about 47 bits, three characters
 * wider than a single lane.
 *
 * The width is a measured trade, not a guess, and the measurement corrected an
 * earlier version of this function. A single 32-bit lane collides at the rate
 * the birthday bound predicts — 4 collisions over 150,000 distinct declaration
 * blocks, 17 over 400,000 — and a collision here is not a retried lookup, it is
 * two different rules sharing a class name, which renders as silently wrong
 * styling with nothing to log. That argued for widening.
 *
 * But the cost scales with the number of DISTINCT rules on a page, and so does
 * the benefit, and they do not meet where the first fix assumed. Two FULL lanes
 * (six extra characters) were measured against a single lane, gzipped:
 *
 *     distinct rules     cost of two full lanes     one-lane collision risk
 *                 20                     +10.1%                  0.0000044%
 *                100                     +17.9%                  0.00012%
 *               1000                     +48.6%                  0.0116%
 *
 * Ten to eighteen per cent of a page's compressed weight, to close a risk near
 * one in a million, is the wrong trade at the scale real pages occupy. Three
 * digits of the second lane keep the risk negligible where it actually matters
 * — about 0.006% at 150,000 rules, where one lane was already failing — at half
 * the byte cost.
 *
 * The client mints the same names from CLIENT_HASH_SOURCE in css.js, which must
 * stay arithmetically identical to this; test/test-css.js asserts the two agree
 * over a corpus. The padding idiom below is deliberately ES5 — `padStart` is
 * not, and the client copy has to be able to spell it the same way.
 */
function hash(str) {
  let h1 = 2166136261;
  let h2 = 2166136387;
  ...
  // Low digits, not high: a small lane-2 value renders as few base-36 digits, so
  // padding and taking the leading three would hand back "000" and throw the
  // entropy away.
  return (h1 >>> 0).toString(36) + ('00' + (h2 >>> 0).toString(36)).slice(-3);
}
...
/* ---- CSS ----
   Declaration compilation, value sanitisation, name and selector validation,
   the scoped class hash and rule de-duplication all live in ./css.js, which is
   the single source of truth for every stylesheet byte the library emits.
   It requires this module for toKebab() and hash(), so nothing here may require
   it back. ---- */

/* ---- Deprecated API aliases ----
 *
 * Several operations were reachable under two or three names — `attr()` and
 * `attribute()`, `create()` and `createElement()` and `child()`, `globalCss()`
 * and `globalStyle()`, `sharedClass()` and `defineClass()`. Every one of them is
 * a second name for a single behaviour, which is a cost paid by every reader:
 * the docs have to explain both, and each new caller has to guess whether the
 * difference is real.
 *
 * The aliases still work and still do exactly what they did, so no existing page
 * changes. They announce themselves once each in dev mode — once per name, not
 * once per call, because a render loop would otherwise bury the message it is
 * trying to deliver — and are silent in prod.
 */
const warnedDeprecations = new Set();

function warnDeprecated(oldName, newName, note = '') {
  const { CONFIG } = require('./config');
  if (CONFIG.mode !== 'dev' || warnedDeprecations.has(oldName)) return;
  warnedDeprecations.add(oldName);
  console.warn(
    `[BuildHTML W_DEPRECATED] ${oldName} is deprecated — use ${newName} instead.` +
    `${note ? ' ' + note : ''} The alias still behaves identically and is scheduled for removal in the next major version.`
  );
}
...
// DOCUMENT, so HTML-escaping — which makes every other attribute safe — is what
// delivers the payload: the parser entity-decodes the value first, turning
// `&lt;script&gt;` back into a live `<script>` inside the frame.
//
// classifyBindableProp() has always refused it for bindProp(), and both the
// README and the type declarations state it is refused. Only the static path
// never enforced it, so `attr('srcdoc', untrusted)` rendered an executable
// frame. Blocked here so the one rule covers attr(), setAttrs(), builder
// `attrs`, template attributes and the _mkEl client alike.
//
// Deliberately not offered as an opt-in: raw markup already has documented
// homes — appendUnsafe(), raw() and the builder `html` field — and adding a
// second unsafe door for one attribute would be a worse trade than telling the
// caller to write the whole element through one of those.
const MARKUP_SINK_ATTR_SOURCE = '^srcdoc$';
...
function clientAttrKeyValidatorBody(keyExpr = 'k') {
  return `return /${ATTR_KEY_SOURCE}/.test(${keyExpr})`
    + `&&!/${EVENT_ATTR_SOURCE}/i.test(${keyExpr})`
    + `&&!/${MARKUP_SINK_ATTR_SOURCE}/i.test(${keyExpr});`;
}
...
function isValidAttrKey(key) {
  ...
  if (new RegExp(MARKUP_SINK_ATTR_SOURCE, 'i').test(key)) return false;
...
module.exports = {
  ...
  sanitizeFunctionSource, sanitizeFunctionSourceString, findFreeVariables, minHTML,
  ...
  clientAttrKeyValidatorBody, warnDeprecated,
