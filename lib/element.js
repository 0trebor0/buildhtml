...
const { toKebab, escapeHtml, sanitizeCssValue, sanitizeFunctionSource, isValidAttrKey, safeJsonStringify, hash, VOID_ELEMENTS } = require('./utils');
...
class Element {
  ...
  html() {
    ...
    const ctx = { events: [], states: [], styles: [], seenCss: new Set(), computed: [], stateBindings: [], portals: [], oncreates: [], globalState: {} };
  ...
  bindAttr(stateKey, attrName, fn = (val) => val) {
    ...
    try {
      const fnSource = sanitizeFunctionSource(fn, CONFIG.maxComputedFnSize);
      this._stateBindings.push({ stateKey, id: this.attrs.id, templateFn: fnSource, bindType: 'attr', attrName: safeAttrName });
      this.hydrate = true;
    } catch (err) {
