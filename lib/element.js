...
const {
  ...
  isValidCssProperty, compileScopedRule, compileNestedRules, RuleSet
} = require('./css');
...
class Element {
  ...
  clone() {
    ...
    for (const k of Object.keys(this.attrs)) el.attrs[k] = this.attrs[k];
  ...
  attr(key, value) {
    ...
    // attr('__proto__', {...}) replaced the prototype of this element's attrs
    // object and every enumerable property on it was then rendered as an
    // attribute. One guard on one of two twin paths is how that survived.
  ...
  data(obj) {
    if (obj && typeof obj === 'object') {
      for (const k of Object.keys(obj)) this.attrs['data-' + toKebab(k)] = obj[k];
    }
    return this;
  }
  ...
  aria(obj) {
    if (obj && typeof obj === 'object') {
      for (const k of Object.keys(obj)) this.attrs['aria-' + toKebab(k)] = obj[k];
    }
    return this;
  }
   ...
   * parses in every browser rather than depending on native CSS nesting.
   */
  css(s) {
    if (!s || typeof s !== 'object') return this;
    const rule = compileNestedRules(s, { prefix: 'c', onInvalid: (k) => warnInvalidCss('css', k) });
    if (!rule) return this;
    ...
    for (const [key, css] of rule.rules) this._cssRules.add(key, css);
    return this;
  }
  ...
  style(prop, value) {
    ...
    const parts = [];
    for (const k of Object.keys(this._inlineStyles)) parts.push(`${k}:${this._inlineStyles[k]}`);
  ...
  classMap(map) {
    if (map && typeof map === 'object') {
      for (const k of Object.keys(map)) { if (map[k]) this._addClassName(k); }
    }
    return this;
  }
  ...
  /**
   * Styles for any pseudo-class — the primitive the named helpers below are
   * built from.
   *
   *   el.pseudoClass('hover', { color: 'blue' })
   ...
   *
   * This exists so the API stops needing a new method per pseudo-class. The six
   * named helpers cover the states most pages use and are staying; anything
   * else — `:checked`, `:focus-visible`, `:nth-of-type()`, `:is()` — is reachable
   * here rather than by adding a seventh, an eighth and a ninth method.
   *
   * The name is validated as an identifier plus an optional argument, the same
   * check every helper below already goes through.
   */
  pseudoClass(name, rules) { return this._pseudoClass(name, rules); }
  ...
  supports(condition, rules) {
    if (!rules || typeof rules !== 'object') return this;
    const rule = compileScopedRule(rules, {
      prefix: 's', supports: condition, onInvalid: (k) => warnInvalidCss('supports', k),
    });
    if (!rule) { warnInvalidCss('supports', condition); return this; }
    this._addClassName(rule.className);
    this._cssRules.add(rule.className, rule.css);
    return this;
  }
  ...
  /**
   * @container block scoped to this element.
   *   el.containerQuery('(min-width: 20em)', { padding: '2rem' })
   *   el.containerQuery('sidebar (min-width: 20em)', { padding: '2rem' })
   *
   * Named `containerQuery` rather than `container` because `container()` is
   * already a layout helper on this prototype — and because `applyShortcuts()`
   * runs after the class body, a clash would not have been an error, it would
   * have silently replaced this method.
   *
   * A container query resolves against the nearest ancestor that declares
   * `container-type`, which is an ordinary declaration — set it with
   * `css({ containerType: 'inline-size' })` on that ancestor. Nothing here can
   * check that it exists, and a query with no container simply never matches.
   */
  containerQuery(query, rules) {
    if (!rules || typeof rules !== 'object') return this;
    const rule = compileScopedRule(rules, {
      prefix: 'q', container: query, onInvalid: (k) => warnInvalidCss('container', k),
    });
    if (!rule) { warnInvalidCss('container', query); return this; }
    this._addClassName(rule.className);
    this._cssRules.add(rule.className, rule.css);
    return this;
  }
  ...
  /* Six one-property aliases over style(), each saving nothing over the
   * primitive it calls — `opacity(0.5)` against `style('opacity', 0.5)` — while
   * standing as an invitation to add a seventh for the next property someone
   * wants. That is the growth this deprecation stops; `style()` and `css()`
   * already cover every property, including the ones these never got around to.
   *
   * `size()`, `transition()`, `transform()` and `animate()` below are NOT
   * deprecated: each builds a composite value or sets more than one property,
   * so they are doing work rather than renaming it.
   */

  /** @deprecated Use style('opacity', n). */
  opacity(n) { warnDeprecated('Element.opacity()', "Element.style('opacity', n)"); return this.style('opacity', String(n)); }
  /** @deprecated Use style('z-index', n). */
  zIndex(n) { warnDeprecated('Element.zIndex()', "Element.style('z-index', n)"); return this.style('z-index', String(n)); }
  /** @deprecated Use style('cursor', type). */
  cursor(type) { warnDeprecated('Element.cursor()', "Element.style('cursor', type)"); return this.style('cursor', type); }
  /** @deprecated Use style('overflow', value). */
  overflow(value) { warnDeprecated('Element.overflow()', "Element.style('overflow', value)"); return this.style('overflow', value); }
  /** @deprecated Use style('display', value). */
  display(value) { warnDeprecated('Element.display()', "Element.style('display', value)"); return this.style('display', value); }
  /** @deprecated Use style('position', value). */
  position(value) { warnDeprecated('Element.position()', "Element.style('position', value)"); return this.style('position', value); }
