...
const { toKebab, normalizeTagName, escapeHtml, sanitizeCssValue, sanitizeFunctionSource, isValidAttrKey, safeJsonStringify, hash, VOID_ELEMENTS, compileCssDeclarations, warnInvalidCss, isValidCssProperty, classifyBindableProp, normalizeEventOptions } = require('./utils');
...
class Element {
  ...
  css(s) {
    ...
    const rules = compileCssDeclarations(s, (k) => warnInvalidCss('css', k));
  ...
  style(prop, value) {
    ...
    if (typeof prop === 'object') {
      for (const decl of compileCssDeclarations(prop, (k) => warnInvalidCss('style', k))) {
        const idx = decl.indexOf(':');
        this._inlineStyles[decl.slice(0, idx)] = decl.slice(idx + 1);
      }
  ...
  pseudo(which, rules) {
    ...
    const cssRules = compileCssDeclarations(rules, (k) => warnInvalidCss('css', k));
