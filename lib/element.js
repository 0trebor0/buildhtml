...
const { toKebab, escapeHtml, sanitizeCssValue, sanitizeFunctionSource, hash, VOID_ELEMENTS } = require('./utils');
...
class Element {
  ...
  clone() {
    ...
    if (this.events.length > 0) el.events = this.events.map(e => ({ ...e }));
    if (this._stateBindings.length > 0) el._stateBindings = this._stateBindings.map(b => ({ ...b }));
  ...
  html() {
    ...
    const ctx = { events: [], states: [], styles: [], seenCss: new Set(), computed: [], stateBindings: [], oncreates: [], globalState: {} };
  ...
  bind(stateKey, templateFn = (val) => val) {
    ...
    try {
      const fnSource = typeof templateFn === 'function'
        ...
        : '(val) => val';
      this._stateBindings.push({ stateKey, id: this.attrs.id, templateFn: fnSource, bindType: 'text' });
      this.hydrate = true;
    } catch (err) {
  ...
  bindShow(stateKey, fn = (val) => val) {
    if (!this.attrs.id) this.id();
    try {
      const fnSource = sanitizeFunctionSource(fn, CONFIG.maxComputedFnSize);
      this._stateBindings.push({ stateKey, id: this.attrs.id, templateFn: fnSource, bindType: 'show' });
      this.hydrate = true;
    } catch (err) {
  ...
  bindClass(stateKey, fn) {
    if (!this.attrs.id) this.id();
    try {
      const fnSource = sanitizeFunctionSource(fn, CONFIG.maxComputedFnSize);
      this._stateBindings.push({ stateKey, id: this.attrs.id, templateFn: fnSource, bindType: 'class' });
      this.hydrate = true;
    } catch (err) {
  ...
  bindAttr(stateKey, attrName, fn = (val) => val) {
    if (!this.attrs.id) this.id();
    try {
      const fnSource = sanitizeFunctionSource(fn, CONFIG.maxComputedFnSize);
      this._stateBindings.push({ stateKey, id: this.attrs.id, templateFn: fnSource, bindType: 'attr', attrName });
      this.hydrate = true;
    } catch (err) {
  ...
  bindStyle(stateKey, fn) {
    if (!this.attrs.id) this.id();
    try {
      const fnSource = sanitizeFunctionSource(fn, CONFIG.maxComputedFnSize);
      this._stateBindings.push({ stateKey, id: this.attrs.id, templateFn: fnSource, bindType: 'style' });
      this.hydrate = true;
    } catch (err) {
  ...
  bindProp(stateKey, prop, fn = (val) => val) {
    if (!this.attrs.id) this.id();
    try {
      const fnSource = sanitizeFunctionSource(fn, CONFIG.maxComputedFnSize);
      this._stateBindings.push({ stateKey, id: this.attrs.id, templateFn: fnSource, bindType: 'prop', prop });
      this.hydrate = true;
    } catch (err) {
  ...
  bindInput(stateKey) {
    ...
    this.events.push({ event: 'input', id: this.attrs.id, fn: `function(){State[${JSON.stringify(stateKey)}]=this.value;}` });
    this.hydrate = true;
    return this;
  }
  ...
  on(ev, fn) {
    try {
      ...
      if (!this.attrs.id) this.id();
      this.events.push({ event: ev, id: this.attrs.id, fn: fnSource });
      this.hydrate = true;
    } catch (err) {
  ...
  bindState(target, ev, fn) {
    try {
      ...
      this.events.push({ event: ev, id: this.attrs.id, targetId: target.attrs.id, fn: fnSource });
      this.hydrate = true;
    } catch (err) {
  ...
  renderFragment() {
    ...
    const ctx = {
      ...
      computed: [], stateBindings: [], oncreates: [], globalState: {}
    };
