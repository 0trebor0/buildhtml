...
const { isValidTagName, toKebab } = require('./utils');

/**
 * Report a malformed template line. The parser recovers rather than throwing —
 * a bad line still produces output — so without this the caller has no signal
 * that attributes, text, or a whole directive were dropped.
 */
function warnTemplateSyntax(message) {
  if (CONFIG.mode === 'dev') console.warn(`[BuildHTML W_TEMPLATE_SYNTAX] ${message}`);
}
...
class TemplateParser {
  ...
  parse(source, variables = {}) {
    // Every entry point — parseTemplate, compileTemplate, renderTemplate,
    // renderFile — funnels through here, so one check covers them all. Without
    // it a non-string surfaced as "source.split is not a function" or "Cannot
    // read properties of null (reading 'split')" from library internals, rather
    // than naming the argument that was wrong.
    if (typeof source !== 'string') {
      throw new TypeError(`Template source must be a string, received ${source === null ? 'null' : typeof source}`);
    }
  ...
  _parseGlobal(trimmed) {
    ...
    if (trimmed.startsWith(':global ')) {
      ...
      if (braceIdx === -1) {
        warnTemplateSyntax(`Missing "{" in: ${trimmed} — the rule is ignored.`);
        return null;
      }
  ...
  _parseElement(baseIndent) {
    ...
    // ?if and ?each are routed before this point, so any other "?" line is a typo
    // or a stray ?else; it would otherwise be parsed as an anonymous <div>.
    if (line.startsWith('?')) {
      warnTemplateSyntax(`Unknown directive: ${line} — expected "?if", "?each", or "?else" inside an "?if" block.`);
    }
    ...
    let rest = line;
    let invalidTag = null;
    // A "?" line already warned as an unknown directive above.
    let reported = line.startsWith('?');
    ...
    if (selectorMatch) {
      ...
      // The selector regex accepts names the element constructor rejects — an
      // uppercase tag is the common typo. Left unchecked, normalizeTagName()
      // threw out of renderTemplate() and the rest of the template was lost,
      // contradicting the documented promise that the parser recovers from a
      // malformed line and reports what it dropped. Flagged here and turned
      // into an error node below, after the line's children have been consumed.
      if (tagPart && !isValidTagName(toKebab(tagPart))) invalidTag = tagPart;
    ...
    if (rest.startsWith('[')) {
      ...
      if (closeIdx !== -1) {
        ...
        const parsedData = this._parseAttrString(dataStr);
        // Same reason as the attribute branch below: a data value is content.
        for (const key in parsedData) parsedData[key] = this._interpolate(parsedData[key]);
        node.dataAttrs = parsedData;
    ...
    if (rest.startsWith('(')) {
      ...
      if (closeIdx === -1) {
        warnTemplateSyntax(`Unclosed "(" in: ${line} — attributes and inline text on this line are ignored.`);
        // Already reported; the leftover check below must not report it twice.
        reported = true;
      }
      if (closeIdx !== -1) {
        ...
        for (const key in parsed) {
          if (key.startsWith('@')) {
            // Event values name a function in `variables`, not a value to expand.
            ...
            // Attribute values interpolate like quoted text. Without this,
            // a(href="#{url}") shipped the literal "#{url}" as the href — the
            // template reads correctly and the link is broken. _interpolate()
            // leaves the token in place when no variable matches, so a literal
            // "#{...}" that resolves to nothing still passes through unchanged.
            node.attrs[key] = this._interpolate(parsed[key]);
          }
        }
    ...
    // Anything still unconsumed is not valid syntax after a tag — the selector
    // regex stops at the first invalid character, so "scr<ipt" yielded <scr> and
    // dropped the rest of the line without a word. The element still renders,
    // which is the documented recovery, but the drop is now reported.
    if (!invalidTag && !reported && rest !== '') {
      warnTemplateSyntax(`Unexpected "${rest}" in: ${line} — ignored. Expected attributes, a CSS block, or quoted text after the tag.`);
    }

    // Parse children
    this._parseChildren(node, baseIndent);

    // Children were consumed first so the rest of the template still parses at
    // the right depth; the malformed line alone is dropped. _buildAstNode()
    // ignores an error node, which is how ?each already recovers.
    if (invalidTag) {
      warnTemplateSyntax(`Invalid element tag: ${invalidTag} — the line is ignored. Tag names are lower-case, e.g. "span" or "my-widget".`);
      return { type: 'error', message: `Invalid element tag: ${invalidTag}` };
    }
  ...
  _parseLoop(baseIndent) {
    ...
    if (!loopMatch) {
      warnTemplateSyntax(`Invalid loop syntax: ${line} — expected "?each item in items" or "?each item, index in items".`);
      return { type: 'error', message: 'Invalid loop syntax: ' + line };
    }
  ...
  _interpolate(str) {
    // A valueless attribute — button(disabled) — parses to boolean true, and
    // attribute values now flow through here as well as quoted text.
  ...
  _parseAttrString(str) {
    const attrs = {};
    // The key allows a colon INSIDE the name, not just the leading one that marks
    // a directive: `xlink:href="/x"` used to match `xlink` alone and leave
    // `:href` as a separate key, which isValidAttrKey() then dropped — losing the
    // href half of a name the library recognises elsewhere (it is in URL_ATTRS).
    //
    // A quoted value may contain an escaped quote. `title="say \"hi\""` used to
    // end the value at the first `\"`, truncating it and parsing the remainder
    // as further attributes, so the markup gained a `hi="true"` nobody wrote.
    const regex = /([:@]?[\w-]+(?::[\w-]+)*)(?:\s*=\s*(?:"((?:\\"|[^"])*)"|'((?:\\'|[^'])*)'|(\S+)))?/g;
    let match;
    while ((match = regex.exec(str)) !== null) {
      const key = match[1];
      let value = match[2] ?? match[3] ?? match[4] ?? true;
      // Only the escaped quote characters are unescaped. A lone backslash is
      // left alone so an attribute holding something like a Windows path keeps it.
  ...
  _astToDocument(ast, variables) {
    ...
    for (const g of ast.globals) {
      ...
      else if (g.type === 'globalStyle') doc.globalCss(g.selector, g.css);
  ...
  _buildElement(doc, parentEl, node, variables) {
    ...
    if (node.bind) {
      if (node.bindFn && typeof variables[node.bindFn] === 'function') {
        el.bind(node.bind, variables[node.bindFn]);
      } else {
        el.bind(node.bind);
      }
    }
  ...
  _buildComponent(doc, parentEl, node, variables) {
    ...
    // components.get() throws for an unregistered name, which is right for the
    // direct component() API but not here: a template promises to recover from a
    // malformed line rather than throw, and an unregistered @Name aborted the
    // whole render — every later line lost to one typo. The line is dropped and
    // reported instead, matching how an invalid tag is handled.
    let el;
    try {
      if (parentEl) {
        // Resolve before creating the child. Building it first left a stray
        // empty <div> behind when the lookup then failed.
        const { fn } = components.get(node.name);
        el = parentEl.child('div');
        fn(el, resolvedProps);
      } else {
        el = doc.component(node.name, resolvedProps);
      }
    } catch (error) {
      warnTemplateSyntax(`${error.message} — the line is ignored.`);
      return null;
    }
 ...
 *
 * The bare readFileSync in renderFile()/compileFile() reported only the Node
 * error, so a missing view surfaced as "ENOENT" with nothing naming the library
 * or the call. templateEngine() already reported failures through its callback;
 * these two propagated raw.
 *
 * NOTE: the path is used exactly as given. Interpolating a request parameter
 * into it — `renderFile('./views/' + req.params.page)` — is a path traversal in
 * your application; resolve and contain the path before calling.
 */
function readTemplateFile(filePath) {
  if (typeof filePath !== 'string' || !filePath) {
    throw new TypeError(`Template path must be a non-empty string, received ${filePath === null ? 'null' : typeof filePath}`);
  }
  try {
    return require('fs').readFileSync(filePath, 'utf-8');
  } catch (err) {
    const error = new Error(`[buildhtml] Could not read the template ${filePath}: ${err.message}`);
    error.code = err.code;
    error.cause = err;
    throw error;
  }
...
function renderFile(filePath, variables = {}) {
  return renderTemplate(readTemplateFile(filePath), variables);
}
...
function compileFile(filePath, variables = {}) {
  return compileTemplate(readTemplateFile(filePath), variables);
}
...
function templateEngine(filePath, options, callback) {
  try {
    const html = renderTemplate(readTemplateFile(filePath), options);
    callback(null, html);
  } catch (err) {
    callback(err);
  }
