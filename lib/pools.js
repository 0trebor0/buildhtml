...
const { normalizeTagName } = require('./utils');
...
function recycle(type, item) {
  ...
  if (type === 'elements' && item instanceof Element) {
    // An element reachable from two parents is walked once per path. Pooling it
    // twice hands one object to two future call sites, which then overwrite each
    // other's tag and children. Marking before recursing also stops a cycle.
...
function resetElement(el, tag, ridGen, stateStore, document) {
  el._pooled = false;
  el.tag = normalizeTagName(tag);
  el._ridGen = ridGen;
  el._stateStore = stateStore;
  el._document = document;
  // Clear attrs (reuse object)
  for (const key in el.attrs) delete el.attrs[key];
  if (!el.attrs) el.attrs = {};
  // Reset arrays (reuse existing)
  if (el.children) el.children.length = 0; else el.children = [];
  if (el.events) el.events.length = 0; else el.events = [];
  if (el._stateBindings) el._stateBindings.length = 0; else el._stateBindings = [];
  if (el._lifecycle) el._lifecycle.length = 0; else el._lifecycle = [];
  if (el._classes) el._classes.length = 0; else el._classes = [];
  if (el._classSet) el._classSet.clear(); else el._classSet = new Set();
  el._cssRules.clear();
  el._state = null;
  el.hydrate = false;
  el._computed = null;
  el._parent = null;
  el._slots = null;
  el._portalTarget = null;
  el._inlineStyles = null;
}
