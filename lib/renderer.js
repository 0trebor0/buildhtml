...
const { escapeHtml, sanitizeFunctionSource, VOID_ELEMENTS, isValidAttrKey, escapeJsString, sanitizeUrl, URL_ATTRS, safeJsonStringify, clientUrlSanitizerBody, classifyBindableProp, listenerOptionsSource, eventModifierSource } = require('./utils');
const { clientStyleApplierSource } = require('./css');
...
function renderNode(n, ctx) {
  ...
  // Own properties only. `for...in` walks the prototype chain, so a single
  // Object.prototype pollution anywhere in the process — from any dependency,
  // not just this library — was rendered as an attribute on every element.
  ...
  // De-duplicated one rule at a time. The whole-string comparison this replaced
  // could not see inside a concatenation, so an element carrying `.css(A).hover(B)`
  // re-emitted A even when a plain `.css(A)` element had already emitted it —
  // the two elements' cssText differed, though one rule was common to both.
  if (n._cssRules && n._cssRules.size > 0) {
    for (const [key, css] of n._cssRules.entries()) {
      if (ctx.seenCss.has(key)) continue;
      ctx.seenCss.add(key);
      ctx.styles.push(css);
    }
  }
...
function compileClient(ctx) {
  ...
  if (hasStateBindings) {
    // The style applier is shared by every style binding on the page and emitted
    // only when one exists, so a page without them compiles byte-identically.
    if (ctx.stateBindings.some(b => (b.bindType || 'text') === 'style')) {
      p.push(clientStyleApplierSource());
    }
    ...
    for (const b of ctx.stateBindings) {
      ...
      if (type === 'show') {
        ...
        // Validates property names and sanitises values, exactly as the server's
        // style() does. The helper is emitted once, above, when any binding needs it.
        updateExpr = `_bhStyle(el,${callBinding});`;
      } else if (type === 'prop') {
  ...
  if (hasEvents) {
    ...
    for (const e of ctx.events) {
      ...
      // Both are built from normalizeEventOptions() output, so they only ever
      // contain the literal `true` — nothing a caller supplied reaches the script.
      // Absent options produce '' and leave the emitted call byte-identical to
      // what a page without options has always compiled to.
      const listenerOptions = listenerOptionsSource(e.options);
      const modifiers = eventModifierSource(e.options);
      p.push(
        `(function(){var el=getById("${safeId}");`,
        `if(el)try{var fn=${fnSource};el.addEventListener("${safeEvent}",function(event){${modifiers}try{var result=fn.call(el,event,window.State,el,${contextSource});if(result&&typeof result.then==="function")result.catch(function(error){reportClientError({type:"event:${safeEvent}",elementId:"${safeId}",tag:el.tagName.toLowerCase()},error);});}catch(error){reportClientError({type:"event:${safeEvent}",elementId:"${safeId}",tag:el.tagName.toLowerCase()},error);}}${listenerOptions});}catch(err){reportClientError({type:"event:${safeEvent}",elementId:"${safeId}",tag:el.tagName.toLowerCase()},err);}`,
        '})();'
      );
    }
    p.push('};');
