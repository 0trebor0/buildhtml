...
function compileClient(ctx) {
  ...
  if (!hasStates && !hasComputed && !hasEvents && !hasOncreates && !hasGlobalState && !hasStateBindings && !hasPortals) return '';
  ...
  if (hasGlobalState || hasStateBindings) {
    p.push(
      ...
      'window.State=new Proxy(' + safeJsonStringify(ctx.globalState || {}) + ',{',
