...
const { sanitizeFunctionSource, toKebab, escapeHtml, isValidAttrKey } = require('./utils');
...
function nodeDefToHtml(def) {
  ...
  if (def.aria) {
    for (const k in def.aria) parts.push(' aria-', toKebab(k), '="', escapeHtml(String(def.aria[k])), '"');
  }
...
const MK_EL_SRC =
  'window._mkEl=(function(){' +
    ...
    'function mk(d){' +
      ...
      'if(d.aria){for(var ak in d.aria)e.setAttribute("aria-"+kb(ak),String(d.aria[ak]));} ' +
...
function compileHashRouter(doc, options = {}) {
  ...
  script += 'State[' + JSON.stringify(stateKey) + ']=h;';
...
module.exports = { compileLiveList, compileHashRouter, MK_EL_SRC, nodeDefToHtml };
