...
const {
  sanitizeCssValue, isSafeMediaQuery, isValidClassName, isValidCssCustomProperty,
  isSafeRawCss, compileMediaRule, compileKeyframesRule, warnInvalidCss
} = require('./css');
...
class Document {
  ...
  fromJSON(def) {
    ...
    if (Array.isArray(def.globalStyles)) {
      ...
      for (const sel in def.globalStyles) this.globalCss(sel, def.globalStyles[sel]);
    }
