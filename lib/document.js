...
class Document {
  ...
  fromJSON(def) {
    ...
    if (Array.isArray(def.styles)) for (const s of def.styles) this.head.addStyle(s);
    ...
    if (Array.isArray(def.globalStyles)) {
      for (const rule of def.globalStyles) {
        if (typeof rule === 'string' && !this.head.globalStyles.includes(rule)) {
          this.head.globalStyles.push(rule);
        }
      }
    ...
    if (def.classStyles && typeof def.classStyles === 'object') {
      for (const name in def.classStyles) {
        if (typeof def.classStyles[name] === 'string') this.head.classStyles[name] = def.classStyles[name];
      }
    }
