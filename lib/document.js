...
const {
  createRidGenerator, escapeHtml, sanitizeFunctionSource, sanitizeFunctionSourceString,
  sanitizeUrl, URL_ATTRS, findFreeVariables, isValidAttrKey, toKebab, minHTML, unescapeHtml,
  safeJsonStringify, warnDeprecated
} = require('./utils');
const {
  sanitizeCssValue, isSafeMediaQuery, isSafeAtRulePrelude, isValidClassName,
  isValidCssCustomProperty, isSafeRawCss, compileMediaRule, compileConditionalRule,
  compileLayerRule, compileLayerOrder, compileKeyframesRule, warnInvalidCss
} = require('./css');
...
class Document {
  ...
  bodyCss(rules) {
    if (rules && typeof rules === 'object') {
      for (const k of Object.keys(rules)) this._bodyCss[k] = rules[k];
      this.globalCss('body', this._bodyCss);
    }
    return this;
  }
  ...
  /** A rule for any CSS selector: `globalCss('body > .row', { gap: '8px' })`. */
  globalCss(selector, rules) { this.head.globalCss(selector, rules); return this; }

  /** A reusable class: `sharedClass('btn', { padding: '8px' })` emits `.btn{…}`. */
  sharedClass(name, rules) { this.head.addClass(name, rules); return this; }

  /** @deprecated Use globalCss(). */
  globalStyle(selector, rules) {
    warnDeprecated('Document.globalStyle()', 'Document.globalCss()');
    return this.globalCss(selector, rules);
  }

  /**
   * @deprecated Use sharedClass() for a class name, or globalCss() for a raw
   * selector.
   *
   * This was a third name for two behaviours already reachable: with
   * `isRawSelector` false it is `sharedClass()`, and with it true it is
   * `globalCss()`. A boolean that switches which of two other methods you meant
   * is harder to read at a call site than simply naming the one you want.
   */
  defineClass(selector, rules, isRawSelector = false) {
    warnDeprecated('Document.defineClass()', 'Document.sharedClass() or Document.globalCss()',
      'defineClass(name, rules) is sharedClass(name, rules); defineClass(sel, rules, true) is globalCss(sel, rules).');
    if (!rules || typeof rules !== 'object') return this;
    return isRawSelector ? this.globalCss(selector, rules) : this.sharedClass(selector, rules);
  }

  resetCss() {
    this.globalCss('*,*::before,*::after', { boxSizing: 'border-box', margin: '0', padding: '0' });
    this.globalCss('body', { lineHeight: '1.5', WebkitFontSmoothing: 'antialiased' });
    this.globalCss('img,picture,video,canvas,svg', { display: 'block', maxWidth: '100%' });
    this.globalCss('input,button,textarea,select', { font: 'inherit' });
    return this;
  }
  ...
  states(obj) {
    if (obj && typeof obj === 'object') {
      for (const k of Object.keys(obj)) this._globalState[k] = obj[k];
    }
    return this;
  }
  ...
  createElement(tag) {
    warnDeprecated('Document.createElement()', 'Document.create()');
    return this.create(tag);
  }

  /** @deprecated Use create(). */
  child(tag) {
    warnDeprecated('Document.child()', 'Document.create()',
      'Element.child() is unaffected — it stays the way to add a child to an element.');
    return this.create(tag);
  }
  ...
  build(defs) {
    // One list so a single definition behaves exactly like a one-element array.
    // These diverged: the array branch handled a bare string while the single
    // branch accepted only objects, so build('hi') and build(42) rendered
    // nothing at all.
    const list = Array.isArray(defs) ? defs : [defs];
    for (const def of list) {
      if (def == null) continue;
      // Text at document level has no parent element to hold it, so it goes
      // straight into the body. buildNode() would reach for parentEl.text(),
      // which a Document does not have — that threw rather than rendering.
      if (typeof def === 'string') { this.body.push(escapeHtml(def)); continue; }
      if (typeof def !== 'object') { this.body.push(escapeHtml(String(def))); continue; }
      if (def.type === 'text' && 'content' in def) { this.body.push(escapeHtml(def.content)); continue; }
      buildNode(this, def);
    }
    return this;
  }
  ...
  keyframes(name, frames) {
    ...
    // so both are validated — by the shared compiler, which is also what
    // Element.media() and the scoped rule builders use.
    const rule = compileKeyframesRule(name, frames, (k) => warnInvalidCss('keyframes', k));
    if (!rule) {
      if (!isValidClassName(name)) warnInvalidCss('keyframes', name);
      return this;
    }
  ...
  mediaQuery(query, selectorRules) {
    ...
    // Same compiler as Element.media(), so one query is judged one way.
    const rule = compileMediaRule(query, selectorRules, (k) => warnInvalidCss('mediaQuery', k));
    if (!rule) {
      if (!isSafeMediaQuery(query)) warnInvalidCss('mediaQuery', query);
      return this;
    }
    this.head.globalStyles.push(rule);
    return this;
  ...
  cssVar(name, value) {
    ...
    const parts = [];
    for (const k of Object.keys(this._cssVars)) parts.push(`${k}:${this._cssVars[k]}`);
  ...
  cssVars(obj) {
    if (obj && typeof obj === 'object') {
      for (const k of Object.keys(obj)) this.cssVar(k, obj[k]);
    }
    return this;
  }

  /**
   * @supports block: rules that apply only where the browser supports a feature.
   *   doc.supports('(display: grid)', { '.layout': { display: 'grid' } })
   */
  supports(condition, selectorRules) {
    if (!condition || !selectorRules || typeof selectorRules !== 'object') return this;
    const rule = compileConditionalRule('supports', condition, selectorRules,
      (k) => warnInvalidCss('supports', k));
    if (!rule) {
      if (!isSafeAtRulePrelude(condition)) warnInvalidCss('supports', condition);
      return this;
    }
    this.head.globalStyles.push(rule);
    return this;
   ...
   *
   * Named `containerQuery` to match `mediaQuery`, and because `container()` is
   * already a layout helper applied to this prototype by applyShortcuts().
   *
   * The container itself is declared with an ordinary `container-type`
   * declaration on the ancestor; a query with no container never matches.
   */
  containerQuery(query, selectorRules) {
    if (!query || !selectorRules || typeof selectorRules !== 'object') return this;
    const rule = compileConditionalRule('container', query, selectorRules,
      (k) => warnInvalidCss('container', k));
    if (!rule) {
      if (!isSafeAtRulePrelude(query)) warnInvalidCss('container', query);
      return this;
    }
    this.head.globalStyles.push(rule);
    return this;
  ...
  /**
   * @layer block: rules whose precedence is decided by layer order rather than
   * by specificity or source position.
   *   doc.layer('base', { body: { margin: '0' } })
   *
   * Declaring a layer with no rules is meaningful — it fixes the name's position
   * — so `doc.layer('base')` emits `@layer base{}`.
   */
  layer(name, selectorRules) {
    const rule = compileLayerRule(name, selectorRules, (k) => warnInvalidCss('layer', k));
    if (!rule) { warnInvalidCss('layer', name); return this; }
    this.head.globalStyles.push(rule);
    return this;
  }

  /**
   * `@layer a, b, c;` — the statement that fixes layer precedence, earliest
   * name lowest. Emit it before the layers themselves; order is preserved
   * exactly as given.
   *   doc.layerOrder('reset', 'base', 'components', 'utilities')
   */
  layerOrder(...names) {
    const flat = names.length === 1 && Array.isArray(names[0]) ? names[0] : names;
    const rule = compileLayerOrder(flat, (k) => warnInvalidCss('layerOrder', k));
    if (!rule) return this;
    this.head.globalStyles.push(rule);
    return this;
  }
   ...
   * @param {boolean} [options.callbacks=true] - When false, every callback-bearing
   *   field is DROPPED rather than screened: `events`, `stateBindings`, `computed`,
   *   `lifecycle`, `liveList`, the `on`/`bind`/`onMount`/`onUpdate`/`onDestroy`/`setup`
   *   node keys, and `oncreateCallbacks`. Restoring a payload you did not produce is
   *   otherwise equivalent to running its JavaScript in your page — the screening in
   *   builder.js catches malformed source, not hostile source, and a denylist cannot
   *   be made to. Markup, text, attributes, classes and CSS still restore normally.
   * @returns {Document} this
   */
  fromJSON(def, options = {}) {
    if (!def || typeof def !== 'object') return this;
    const allowCallbacks = !(options && options.callbacks === false);
    ...
    if (Array.isArray(def.globalStyles)) {
      ...
      for (const sel of Object.keys(def.globalStyles)) this.globalCss(sel, def.globalStyles[sel]);
    }
    if (def.sharedClasses && typeof def.sharedClasses === 'object') {
      for (const name of Object.keys(def.sharedClasses)) this.sharedClass(name, def.sharedClasses[name]);
    }
    // toJSON form: class name -> compiled declaration string.
    if (def.classStyles && typeof def.classStyles === 'object') {
      for (const name of Object.keys(def.classStyles)) {
    ...
    if (def.keyframes && typeof def.keyframes === 'object') {
      for (const name of Object.keys(def.keyframes)) this.keyframes(name, def.keyframes[name]);
    }
    ...
    if (Array.isArray(def.oncreateCallbacks) && allowCallbacks) {
    ...
    if (def.body) {
      ...
      // Same one-decision-at-the-top rule as trustedCss: a per-node opt-in inside
      // an untrusted payload would let the payload grant itself the exemption.
      this._dropCallbacks = !allowCallbacks;
      try {
        this.build(def.body);
      } finally {
   ...
   *
   * A failed write is re-thrown with the path and the underlying reason. The bare
   * `writeFileSync` this replaced surfaced as "ENOENT: no such file or directory,
   * open '...'" with no indication that buildhtml was the caller or that a render
   * had already been consumed by it.
   */
  save(path) {
    ...
    try {
      ...
      const error = new Error(`[Document] Could not write the rendered page to ${path}: ${err.message}`);
      error.code = err.code;
      error.cause = err;
      throw error;
    }
   ...
   *
   * Work happens on demand, at TOP-LEVEL BODY NODE granularity: the head is
   * yielded first, then one chunk per node in `this.body`, stopping whenever the
   * consumer's buffer is full. A slow client therefore applies real backpressure
   * between top-level nodes.
   *
   * That granularity is the whole story, and it is easy to misread. renderNode()
   * renders a subtree in a single recursive call, so a page assembled under one
   * root element — the usual shape — emits its entire body as ONE chunk, and
   * streaming buys almost nothing over render(). Measured on 20,000 paragraphs:
   * wrapped in a root div, 3 chunks with the first at 81% of total time; as
   * 20,000 top-level nodes, 20,002 chunks with the first at 10%.
   *
   * So streaming pays off when the body holds many top-level nodes. Making it
   * incremental *within* a subtree would mean a generator-based renderNode(),
   * which is a larger change than this comment should promise.
   *
   * Usage:
   *   app.get('/', (req, res) => {
  ...
  clear() {
    ...
    this.body.length = 0;
    for (const key of Object.keys(this._stateStore)) delete this._stateStore[key];
  ...
  _renderHtmlAttrs() {
    const parts = [];
    // Own properties only — see the note in renderer.js: an inherited key from a
    // polluted Object.prototype would otherwise be rendered as an attribute.
    for (const k of Object.keys(this._htmlAttrs)) {
  ...
  _renderBodyAttrs() {
    ...
    const parts = [];
    for (const k of Object.keys(allAttrs)) {
  ...
  render() {
    ...
    try {
      ...
      // renderNode() walks the tree recursively, so a document nested deeper than
      // the JS stack allows surfaced as a bare "Maximum call stack size exceeded"
      // pointing into library internals, with nothing to say which document or
      // what to do about it.
      //
      // Reported, not prevented: builder.js declines to impose a depth cap for
      // the reason recorded there — the real ceiling varies with platform, Node
      // version and --stack-size, so any fixed limit would refuse trees that
      // render fine today. Naming the cause is the improvement available without
      // turning "works here" into "always refused".
      if (err instanceof RangeError && /call stack/i.test(err.message)) {
        const error = new RangeError(
          '[Document] The element tree is nested too deeply to render: renderNode() '
          + 'exhausted the JavaScript stack. Flatten the tree, or raise the limit with '
          + '--stack-size. The document has not been cleared, so it can be inspected.'
        );
        error.cause = err;
        throw error;
      }
...
applyShortcuts(Document.prototype, 'create');
