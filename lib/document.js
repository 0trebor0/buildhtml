...
class Document {
   ...
   * Sends <head> immediately so the browser can start loading
   * resources while the body is still being built.
   *
   * Usage:
   *   app.get('/', (req, res) => {
   ...
   *   });
   *
   * Unlike render(), streaming emits <style> after the body (the head is already
   * sent) and does not apply prod minification or the response cache, because
   * both need the whole document before the first byte can go out.
   */
  renderStream() {
    const { Readable } = require('stream');
    const stream = new Readable({ read() {} });
    // Mirror what is streamed so output()/save() still see the rendered page;
    // clear() runs below, so re-rendering later would produce an empty document.
    const sent = [];
    const send = (part) => { sent.push(part); stream.push(part); };
    ...
    const ctx = {
      events: getPooled('arrays'), states: getPooled('arrays'), styles: [],
      seenCss: new Set(), computed: getPooled('arrays'), stateBindings: getPooled('arrays'),
      lifecycles: getPooled('arrays'), portals: getPooled('arrays'), oncreates: this._oncreateCallbacks,
      callbackSources: this._callbackSources, registrationErrors: this._registrationErrors,
      globalState: this._globalState, nonce: this._nonce
    };

    try {
      const headHTML = this.head.render();
      const rawHead = this._rawHeadContent.join('');

      // Send head immediately — browser starts loading CSS/fonts now
      send(`<!DOCTYPE html><html${this._renderHtmlAttrs()}><head>${headHTML}${rawHead}</head><body${this._renderBodyAttrs()}>`);
      ...
      if (ctx.styles.length > 0) send(`<style${na}>${ctx.styles.join('')}</style>`);

      const clientJS = compileClient(ctx);
      if (clientJS) send(`<script${na}>${clientJS}</script>`);

      if (this._inlineScripts.length > 0) {
        send(this._inlineScripts.map(s => `<script${na}>${s}</script>`).join(''));
      }

      send('</body></html>');
      stream.push(null);
      this._lastRendered = sent.join('');
    } catch (err) {
      stream.destroy(err);
    } finally {
      recycle('arrays', ctx.events); recycle('arrays', ctx.states);
      recycle('arrays', ctx.computed); recycle('arrays', ctx.stateBindings);
  ...
  toJSON() {
    const serialize = (el) => {
      ...
      if (el.events?.length > 0) {
        s.events = el.events.map(e => ({ event: e.event, id: e.id, targetId: e.targetId, fn: e.fn, context: e.context }));
      }
