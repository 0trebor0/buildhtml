...
const {
  ...
  sanitizeUrl, isValidAttrKey, toKebab, minHTML, unescapeHtml
} = require('./utils');
...
class Document {
  ...
  renderStream() {
    ...
    const ctx = {
      ...
      lifecycles: getPooled('arrays'), portals: getPooled('arrays'), oncreates: this._oncreateCallbacks, globalState: this._globalState, nonce: this._nonce
    };
  ...
  toJSON() {
    const serialize = (el) => {
      ...
      if (el.events?.length > 0) {
        s.events = el.events.map(e => ({ event: e.event, id: e.id, targetId: e.targetId, fn: e.fn }));
      }
