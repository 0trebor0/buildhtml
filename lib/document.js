...
class Document {
  ...
  renderStream() {
    ...
    try {
      ...
      stream.push(`<!DOCTYPE html><html${this._renderHtmlAttrs()}><head>${headHTML}${rawHead}</head><body${this._renderBodyAttrs()}>`);
      ...
      if (ctx.styles.length > 0) stream.push(`<style${na}>${ctx.styles.join('')}</style>`);

      const clientJS = compileClient(ctx);
      if (clientJS) stream.push(`<script${na}>${clientJS}</script>`);

      if (this._inlineScripts.length > 0) {
        stream.push(this._inlineScripts.map(s => `<script${na}>${s}</script>`).join(''));
      }
  ...
  clear() {
    ...
    this._cssVarsRuleIdx = undefined;
