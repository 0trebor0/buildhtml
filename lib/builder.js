...
const { sanitizeFunctionSourceString } = require('./utils');
...
function buildNode(parentEl, def) {
  ...
  if (def.events && Array.isArray(def.events)) {
    for (const e of def.events) {
      if (e && e.event && e.fn) {
        el.events.push({ event: e.event, id: e.id || el.attrs.id, targetId: e.targetId, fn: e.fn, context: e.context });
        el.hydrate = true;
      }
    }
  }
  ...
  if (def.lifecycle && Array.isArray(def.lifecycle)) {
    for (const hook of def.lifecycle) {
      ...
      try {
        ...
        if (typeof doc._recordCallbackFailure === 'function') doc._recordCallbackFailure(`lifecycle:${hook.type}`, err, el);
        if (CONFIG.mode === 'dev') console.error('[build] Invalid lifecycle source:', err.message);
      }
    }
