 ...
 *
 * What that bar is, precisely — because this comment used to claim more than it
 * delivers. The checks below catch a MALFORMED payload: source that does not
 * parse, an id or event name that is not an identifier, a `context` that is not
 * JSON. They are a denylist for the rest, and a denylist of dangerous patterns
 * is not a sandbox: `({}).constructor.constructor("…")()` reads no differently
 * to it than ordinary code, and neither does `document["cookie"]`.
 *
 * So restoring callbacks from a payload you did not produce is equivalent to
 * running that payload's JavaScript in your page, and no amount of screening
 * here changes that. Callers restoring untrusted JSON should pass
 * `{ callbacks: false }` to fromJSON(), which drops every callback-bearing
 * field instead of screening it — an allowlist of "no executable content at
 * all" is the only boundary that actually holds. SECURITY.md says the same.
 *
...
/**
 * True when this tree is being restored with fromJSON(def, { callbacks: false }).
 * Read from the owning document rather than threaded through every call, exactly
 * as the `trustedCss` decision is.
 */
function callbacksDropped(node) {
  const doc = node && (node._document || node);
  return !!(doc && doc._dropCallbacks);
}
...
function buildNodeInner(parentEl, def) {
  ...
  if (def.liveList && !callbacksDropped(parentEl)) {
  ...
  if (def.bind && !callbacksDropped(el)) {
  ...
  if (def.on && typeof def.on === 'object' && !callbacksDropped(el)) {
    for (const ev of Object.keys(def.on)) el.on(ev, def.on[ev]);
  }
  // toJSON serialized events array — source strings, revalidated before use
  if (def.events && Array.isArray(def.events) && !callbacksDropped(el)) {
  ...
  if (def.stateBindings && Array.isArray(def.stateBindings) && !callbacksDropped(el)) {
  ...
  const keepCallbacks = !callbacksDropped(el);
  if (typeof def.onMount === 'function' && keepCallbacks) el.onMount(def.onMount);
  if (def.onUpdate && keepCallbacks) {
  ...
  if (typeof def.onDestroy === 'function' && keepCallbacks) el.onDestroy(def.onDestroy);
  if (def.lifecycle && Array.isArray(def.lifecycle) && keepCallbacks) {
  ...
  if (def.computed && typeof def.computed === 'string' && keepCallbacks) {
    try {
  ...
  // Custom setup
  if (typeof def.setup === 'function' && keepCallbacks) {
    def.setup(el);
  }
