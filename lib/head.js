...
const {
  toKebab, escapeHtml, sanitizeCssValue, sanitizeUrl, isValidAttrKey,
  isSafeCssSelector, isValidClassName, compileCssDeclarations, warnInvalidCss
} = require('./utils');
...
class Head {
  ...
  globalCss(selector, rules) {
    ...
    if (!isSafeCssSelector(selector)) { warnInvalidCss('globalCss', selector); return this; }
    const parts = compileCssDeclarations(rules, (k) => warnInvalidCss('globalCss', k));
    if (parts.length > 0) {
      const cssStr = `${selector}{${parts.join(';')};}`;
  ...
  addClass(name, rules) {
    ...
    const parts = compileCssDeclarations(rules, (k) => warnInvalidCss('sharedClass', k));
