...
function createCachedRenderer(builderFn, cacheKeyOrFn, options = {}) {
  ...
  return async (req, res, next) => {
    // Tracked so a rejected render can be evicted before this request returns.
    let awaitedKey = null;
    let awaitedPromise = null;
    try {
      const hasNonce = options.nonce && typeof options.nonce === 'function';
      if (hasNonce) {
      ...
      if (!promise) {
        ...
        promise
          .then((html) => {
            if (inFlightCache.get(key) === promise) getResponseCache().set(key, html);
          })
          .catch(() => {})
          .finally(() => {
      ...
      // The .finally() above settles a microtask after the rejection, so without
      // this a request arriving in that window would await the same rejected
      // promise and inherit the failure instead of rebuilding.
      if (awaitedKey !== null && inFlightCache.get(awaitedKey) === awaitedPromise) {
        inFlightCache.delete(awaitedKey);
      }
...
function clearCache(pattern) {
  ...
  for (const [key] of inFlightCache) {
    if (String(key).includes(pattern)) inFlightCache.delete(key);
  }
}
